...
public final class DefinicaoJogadoresDialog extends JDialog {
    ...
    private static final class ColorItem {
        ...
        ColorItem(String name, Color color, int pinIndex) {
            this.name = name; this.color = color; this.pinIndex = pinIndex;
        }
    ...
    public DefinicaoJogadoresDialog(java.awt.Window owner, AppController controller, int nJogadores) {
        ...
        for (int i = 0; i < nJogadores; i++) {
            ...
            combo.setSelectedIndex(i % COLORS.length); // sugestão sem repetição
            corCombos[i] = combo;
            form.add(combo, gbc);
        }
    ...
    private void addHeader(JPanel panel, GridBagConstraints gbc, String c1, String c2, String c3) {
        gbc.gridx = 0; panel.add(boldLabel(c1), gbc);
        gbc.gridx = 1; panel.add(boldLabel(c2), gbc);
        gbc.gridx = 2; panel.add(boldLabel(c3), gbc);
    }
    ...
    private ColorItem[] buildPalette() {
        ...
        for (int i = 0; i < COLOR_NAMES.length; i++) {
            items[i] = new ColorItem(COLOR_NAMES[i], COLORS[i], i); // i == pinIndex
        }
        return items;
    }
    ...
    // Renderer do combo para exibir uma amostra da cor
